...
With `self.size` now available, we can expose it via Python's standard `__len__` dunder method and build `is_empty` on top of it:
...
These two will be called throughout the rest of our methods, so it's important to have them in place before moving on.
...
We can also implement the `height` method that we defined back in [**section 2-6**](#2-6). Recall that the height of a tree is the length of its longest path. We handle four cases recursively:

- If both children are `None`, this is a leaf — its height contribution is `0`.
- If only one child is `None`, we recurse into the other and add `1` for the current edge.
- If both children exist, we recurse into both and take the `max`, adding `1` for the current edge.
...
def height(self):
    def subtree_height(root):
        ...
        elif root.left is None:   # only right child exists
