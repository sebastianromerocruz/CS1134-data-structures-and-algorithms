...
<p align=center><strong><em>Song of the day</strong></em>: <em><a href="https://youtu.be/qvvhCzDobRE?si=ksRxJdyxi4913o9k"><strong><u>Steal Away</u></strong></a> by Robbie Dupree (1980)</em></p>
...
Think about every data structure we have covered so far: arrays, linked lists, stacks, queues. What do they all have in common? They are all **positional**; to access an element you either need to know *where* it lives (its index or its node reference), or you have to scan through the collection until you find what you're looking for.

That model works great for ordered collections, but real-world problems often don't fit it. Consider a few examples:

- A **grade book**: you want to look up a student's grade by their _name_, not by some arbitrary position.
- A **contacts list**: you want a phone number given a person's name.
- A **cache**: you want to retrieve a stored result given the exact input that produced it.

In all of these cases, you have a _meaningful key_ (the name, the input) and a _corresponding value_ (the grade, the number, the result). You don't care about position—you care about **association**. This is precisely the problem that a **map** solves.

The next ADT that we will cover is called a **map**, and it's one that you are already quite familiar with, as they form the basis of our favourite Python type: the dictionary class (`dict`)! As a general reminder, let's list out some of the properties that Python dictionaries have, as well as the operations we have available to us when we use them:
...
To support `len(m)` and a convenience `is_empty()` check, we delegate directly to the underlying `ArrayList`:
...
Both are Θ(1): the `ArrayList` already tracks its own length, so no traversal is needed.

<a id="2-1-5"></a>

#### Iteration

Iteration over a map should yield its **keys** (matching how Python's `dict` works when you write `for k in d:`). We use `__iter__` with a generator:
